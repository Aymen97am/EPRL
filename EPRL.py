...
class EPRLAgent(Agent):
    ...
    def exploration_policy(self):
        ...
        if self.sampling_rule == "max_diameter":
                    ...
                    for aa in self.Active[hh][ss]:
                        if self.D_hsa[hh, ss, aa] >= maxD:
                            if len(argmax) == 0:
                                ...
                                bb = argmax[0]
                                if self.D_hsa[hh, ss, aa] > maxD or self.N_hsa[hh, ss, aa] < self.N_hsa[hh, ss, bb]:
                                    argmax[0] = aa
            ...
            # whose visits are smaller than the phase index 
            ExpR = np.zeros((H, S, A))
            for hh in range(H):
                for ss in self.Reachable[hh]:
                    for aa in self.Active[hh][ss]:
    ...
    def elimination(self):
        ...
        for hh in range(H):
            for ss in self.Reachable[hh]:
                Eliminate = []
                for aa in self.Active[hh][ss]:
                    # we start by ruling out pairs whose value gaps are sure to be positive
                    if self.UQ_hsa[hh, ss, aa] < self.LV_hs[hh, ss]:
                        Eliminate.append(aa)
                    # then we check the return gaps
                    else:
                        Ur_hsa =  self.R_hat + self.bonus #upper bounds on the reward
